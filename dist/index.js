...
import { yUndoPluginKey as yUndoPluginKey2 } from "@tiptap/y-tiptap";
function getUndoDepth(editor) {
  return yUndoPluginKey2.getState(editor.state).undoManager.undoStack.length;
}
function getRedoDepth(editor) {
  return yUndoPluginKey2.getState(editor.state).undoManager.redoStack.length;
}
