...
var Collaboration = import_core2.Extension.create({
  ...
  // We handle keyboard shortcuts in a different way
...
// src/helpers/undoRedoDepth.ts
var import_y_tiptap4 = require("@tiptap/y-tiptap");
function getUndoDepth(editor) {
  return import_y_tiptap4.yUndoPluginKey.getState(editor.state).undoManager.undoStack.length;
}
function getRedoDepth(editor) {
  return import_y_tiptap4.yUndoPluginKey.getState(editor.state).undoManager.redoStack.length;
}
